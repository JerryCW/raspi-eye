...
2. WHEN 触发的 object key 以 `event.json` 结尾时，THE Inference_Lambda SHALL 从 S3 读取该 `event.json` 文件，解析出 `event_id`、`device_id`、`start_time`、`end_time`、`frame_count`、`kvs_stream_name`、`kvs_region`、`detections_summary`、`snapshots` 等元数据
...
6. THE Inference_Lambda SHALL 将推理结果通过 `update_item` 写回已有的 `raspi-eye-events` DynamoDB 表（PK: device_id + start_time），写入两类字段：(a) event.json 原始字段（前端展示用）：`eventId`、`endTime`、`durationSec`（计算值）、`frameCount`、`kvsStreamName`、`kvsRegion`、`yoloTopClass`（detections_summary 中检测次数最多的类别）、`yoloTopConfidence`（该类别最高置信度）、`thumbnailKey`（第一张截图的完整 S3 key）、`detections_summary`（数值转 Decimal）、`snapshots`；(b) 推理结果字段加 `inference_` 前缀：`inference_species`、`inference_confidence`、`inference_image_key`、`inference_top5`、`inference_latency_ms`；(c) 前端验证字段：`verified`（= inference_reliable）、`species`（reliable 时为物种名，否则为 null）
